...
public class ChessGui extends JPanel implements IPlayerHandler{
	...
	private void changeboardstate() {
		for(int i=0;i<8;i++) {
			for(int j=0;j<8;j++) {
				this.chessGame.pieces.remove(this.chessGame.getNonCapturedPieceAtLocation(i,j));
			}
		}
		...
		for(int z=0;z<fin.length();z++) {
			...
			if(Character.toString(ch).equals("/")) {
				...
				switch(ch){
				case 'R':
				this.chessGame.createAndAddPiece(Piece.COLOR_WHITE, Piece.TYPE_ROOK, i, j);
				break;
				case 'H':
					this.chessGame.createAndAddPiece(Piece.COLOR_WHITE, Piece.TYPE_KNIGHT, i,j);
						break;
						case 'B':
							this.chessGame.createAndAddPiece(Piece.COLOR_WHITE, Piece.TYPE_BISHOP, i,j);
						break;
								case 'Q':
									this.chessGame.createAndAddPiece(Piece.COLOR_WHITE, Piece.TYPE_QUEEN, i,j);
						break;
								case 'K':
									this.chessGame.createAndAddPiece(Piece.COLOR_WHITE, Piece.TYPE_KING, i,j);
						break;
						case 'P':
							this.chessGame.createAndAddPiece(Piece.COLOR_WHITE, Piece.TYPE_PAWN, i,j);
						   break;
				case 'r':
					this.chessGame.createAndAddPiece(Piece.COLOR_BLACK, Piece.TYPE_ROOK, i,j);
				break;
				case 'h':
					this.chessGame.createAndAddPiece(Piece.COLOR_BLACK, Piece.TYPE_KNIGHT, i,j);
						break;
						case 'b':
							this.chessGame.createAndAddPiece(Piece.COLOR_BLACK, Piece.TYPE_BISHOP, i,j);
						break;
								case 'q':
									this.chessGame.createAndAddPiece(Piece.COLOR_BLACK, Piece.TYPE_QUEEN, i,j);
						break;
								case 'k':
									this.chessGame.createAndAddPiece(Piece.COLOR_BLACK, Piece.TYPE_KING, i,j);
						break;
						case 'p':
							this.chessGame.createAndAddPiece(Piece.COLOR_BLACK, Piece.TYPE_PAWN, i,j);
						   break;
				default:
